...
public class DockWorkInformationRequestDao {

    /**
     * 查询数据
     */
    public DockWorkInformation queryData(String appliid) throws SQLException {
        QueryRunner queryRunner = new QueryRunner(JDBCUtils.getDataSource());
        String sql = "select * from edi_v_wh_whzysbd  where APPLIID = ?";
        DockWorkInformation dwi = queryRunner.query(sql, new BeanHandler<DockWorkInformation>
                (DockWorkInformation.class), appliid);
        return dwi;
    }
    ...
    public void saveData(DockWorkInformation dwi) throws SQLException {
        ...
        String sql = "insert into edi_v_wh_whzysbd (APPLIID,LINKMANNM,TELEPHONENUMNB,CONSIGNER,CONSIGNERTELNB,PLANSTARTDATEDT," +
...
// String sql="insert into v_wh_whzysbd ('APPLIID','LINKMANNM','TELEPHONENUMNB','CONSIGNER') values (?,?,?,?)";values
        Object[] params = {dwi.getAPPLIID(), dwi.getLINKMANNM(), dwi.getTELEPHONENUMNB(), dwi.getCONSIGNER(), dwi.getCONSIGNERTELNB(),
                ...
                dwi.getSENDMANNAME(), dwi.getSENDDATEDT(), dwi.getISIN(), dwi.getISAPPEND(), dwi.getAUDITDEPARTID(), dwi.getAUDITDEPARTNAME(),
                dwi.getWORKMANNM(), dwi.getDEAPPLYNOTE(), dwi.getBACKAPPLYNOTE(),
