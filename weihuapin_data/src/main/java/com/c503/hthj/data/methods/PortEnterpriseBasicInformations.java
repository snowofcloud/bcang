...
import com.c503.hthj.data.dao.first.PortEnterpriseBasicInformationRequestDao;
...
import com.c503.hthj.data.utils.PortEnterpriseBasicInformationTransformUtils;
...
public class PortEnterpriseBasicInformations {
    /**
     * 增加日志
     */
    private static Logger log = Logger.getLogger(PortEnterpriseBasicInformations.class);

    public static void method() {

        Timer timer = new Timer();
        //5秒后开始执行，12小时后开始循环执行    //12*60*60*1000
        timer.schedule(new MyTask2(), 5000, 12 * 60 * 60 * 1000);
...
//         System.out.println(ss);
        ...
        System.out.println("港口企业基本信息数据总数:" + count2);
        Integer a = count2 / 2000;
        Integer b = count2 % 2000;
        if (b >= 1) {
            a = a + 1;
        }
        System.out.println("港口企业基本信息总页数:" + a + "页");
        System.out.println("港口企业基本信息最后一页数据个数:" + b + "条");
        /**统计插入数据数量*/
        int count3 = 0;
        for (int j = 1; j <= a; j++) {
            String s = ghsjzxPort.ghsjzxService("HH_ZHGS", "HH_ZHGS",
                    j, 2000, "V_XZXK_GKQYJBXX", "0");
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                ...
                //调用查询工具
                String ywjbxxid = PortEnterpriseBasicInformationTransformUtils.queryData(jsonObject);
//                System.out.println(mapid);
                PortEnterpriseBasicInformationRequestDao portEnterpriseBasicInformationRequestDao =
                        new PortEnterpriseBasicInformationRequestDao();
                PortEnterpriseBasicInformation pebi = null;
                try {
                    pebi = portEnterpriseBasicInformationRequestDao.queryData(ywjbxxid);
                ...
                count++;
                pebi = PortEnterpriseBasicInformationTransformUtils.getData(jsonObject);

                /**对象DockWorkInformation里的数据获取，通过sql存入MySql数据库*/
                try {
                    portEnterpriseBasicInformationRequestDao.saveData(pebi);
                } catch (SQLException e) {
                    e.printStackTrace();
        ...
        System.out.println("港口企业基本信息本次插入" + count3 + "条数据.");
        ...
        System.out.println("港口企业基本信息本次耗时：" + totalHours + "时=" + totalMintues + "分=" + totalSeconds + "s秒=" + total + "毫秒");
