...
public class AllShipInfos {
    ...
    public static void method() {
        ...
        timer.schedule(new MyTask9(), 1000, 12 * 60 * 60 * 1000);
    }

}

class MyTask9 extends TimerTask {
    ...
    public void run() {
        ...
        String ss = port.getAllShipInfo(1, 1, "2000-10-11 00:00:00");//查询所有船舶信息
        ...
        int count3 = 0;
        for (int j = 1; j <= a; j++) {
            String s = port.getAllShipInfo(j, 1000, "2000-10-11 00:00:00");//查询所有船舶信息
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                ...
                //调用查询工具
                String cbdjh = AllShipInfoTransformUtils.queryData(jsonObject);
                AllShipInfoRequestDao allShipInfoRequestDao = new AllShipInfoRequestDao();
                AllShipInfo bis = null;
                try {
                    bis = allShipInfoRequestDao.queryData(cbdjh);
                ...
                bis = AllShipInfoTransformUtils.getData(jsonObject);
                /**对象AllShipInfo里的数据获取，通过sql存入MySql*/
                try {
                    allShipInfoRequestDao.saveData(bis);
