...
import com.c503.hthj.data.dao.thrid.AllShipInfoRequestDao;
import com.c503.hthj.data.dao.thrid.AllWorkRequestDao;
...
import com.c503.hthj.data.utils.AllShipInfoTransformUtils;
import com.c503.hthj.data.utils.AllWorkTransformUtils;
...
public class AllWorks {
    ...
    public static void method() {
        ...
        timer.schedule(new MyTask10(), 1000, 12 * 60 * 60 * 1000);
    }
}

class MyTask10 extends TimerTask {
    ...
    public void run() {
        ...
        String ss = port.getAllWork(1, 10, "2018-10-11 00:00:00");//查询危废作业主表
        ...
        int count3 = 0;
        for (int j = 1; j <= a; j++) {
            String s = port.getAllWork(j, 1000, "2018-10-11 00:00:00");//查询所有船舶信息
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                ...
                //调用查询工具
                String qyid = AllWorkTransformUtils.queryData(jsonObject);
                AllWorkRequestDao allWorkRequestDao = new AllWorkRequestDao();
                AllWork bis = null;
                try {
                    bis = allWorkRequestDao.queryData(qyid);
                ...
                bis = AllWorkTransformUtils.getData(jsonObject);
                /**对象AllShipInfo里的数据获取，通过sql存入MySql*/
                try {
                    allWorkRequestDao.saveData(bis);
