...
//        Preconditions.checkArgument(!Strings.isNullOrEmpty(val), "令牌不存在或已经过期");
    ...
    public void saveToken(AppToken tokenObj) {
        log.debug("save uac : username[{}]-userid[{}]-uac[{}]", tokenObj.getUsername(), tokenObj.getUserId(), tokenObj.getToken());
                                                      //这个tokenObj.getToken()和下面的有什么区别？/**/
        String key = new StringBuilder(appConfig.keyName()).append("-").append(tokenObj.getToken()).toString();
        redisTemplate.opsForValue().set(key, JSON.toJSONString(tokenObj), appConfig.getExpired(), TimeUnit.MINUTES);
    }
    ...
    public void saveUserInfo(AppToken tokenObj) {
        ...
        if (message.getCode() == 0) {
            ...
            //把用户信息和权限保存到data里面
            data.put("rights", rights);           //appConfig.getApiKeyUserId()和token拼接
            String key = new StringBuilder(appConfig.getApiKeyUserId()).append("-").append(token).toString();
            redisTemplate.opsForValue().set(key, JSON.toJSONString(data), appConfig.getExpired(), TimeUnit.MINUTES);
        }
