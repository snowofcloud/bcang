...
*/
HthxMap.BufferSearch = function(opt_options) {

  var options = opt_options || {};
  var map = options.map;
  var displayClass = options.displayClass || "bufferSearch";
  var target = options.target || "bufferSearch";
  //创建控件位置节点
  $("#toolbar").append('<div id="bufferSearch"></div>');
	  ...
	  var strs = '<div class="bufferPan" style="height:0px;">' +
	                 ...
	                 '<div id="lowBuffer">'+
		                 '<div style="width:100%; margin-top:2px; margin-left:2px;">' + 
			                 '<div style="width:100%;">' +
				       	  	  	 '<label style="font-size:1px;">缓冲区:</label>' +		  	  	
				       	  	  	 '<input style="width:63%; height:24px; font-size:13px" type="text" placeholder="单位m" id="btn_searchRadius"> m' +
				       	  	  '</div>' +
				       	  	  '<div id="multiBuffer" class="multiBuffer" style="width:109%; margin-left:-31px; cursor:pointer;">' +
				       	  	  	 '<ul>' +		       	  	  	 	
		             ...
		             '</div>'+
	             '</div>';
	  $(strs).insertBefore(".bufferSearchInactivate");
	  //缓冲区查询
	  $(".mulBuffer").on('click', queryBufferShip);
...
ol.inherits(HthxMap.BufferSearch, ol.control.Control);
