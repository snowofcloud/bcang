...
*/
HthxMap.UserDefinedFacility = function(opt_options) {

  var options = opt_options || {};
  var map = options.map;
  var displayClass = options.displayClass || "userDefinedFacility";
  var target = options.target || "userDefinedFacility";
  //创建控件位置节点
  $("#toolbar").append('<div id="userDefinedFacility"></div>');
  ...
  function init(){
	  ...
	  //只使用多边形换区域时
	 $(".userDefinedFacilityInactivate").on('click',pointMarker);
	 //新增弹框
	// userDefinedFacilityFind.definedFacilityShowPanel("#addUserDefinedFacility-panel","新增自定义设施");
  }
  
  //添加标记
  var pointMarker = new HthxMap.AddFacilityMarker({
      displayClass:'addFacilityMarker',
      target:'addFacilityMarker',
      map:map
  });
...
ol.inherits(HthxMap.UserDefinedFacility, ol.control.Control);
