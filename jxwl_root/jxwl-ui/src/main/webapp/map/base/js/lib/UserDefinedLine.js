...
*/
HthxMap.UserDefinedLine = function(opt_options) {

  var options = opt_options || {};
  var map = options.map;
  var displayClass = options.displayClass || "userDefinedLine";
  var target = options.target || "userDefinedLine";
  //创建控件位置节点
  $("#toolbar").append('<div id="userDefinedLine"></div>');
  ...
  function init(){
	  ...
	  //只使用多边形换线条时
	 $(".userDefinedLineInactivate").on('click',{id:"linebuffer"},queryUserDefinedLine);
	  //添加缓冲区弹出框
  }
...
ol.inherits(HthxMap.UserDefinedLine, ol.control.Control);
