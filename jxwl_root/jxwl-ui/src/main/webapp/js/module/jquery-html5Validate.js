...
(function ($, undefined) {
    ...
    OBJREG = {
        ...
        ZIPCODE: "^\\d{6}$",
        /*统一修改规则*/
       //   /^((0\d{2,3})-)(\d{7,8})(-(\d{1,4}))?$|^(0\d{2,3})?(\d{7,8})$|^(\d{1,4})$/
        PHONE: "^\\d*(\\(\\d+\\))*(((\\(\\d+\\)|\\d)+)(-)((\\(\\d+\\)|\\d)+))*(\\(\\d+\\))*\\d*$", /*(联系电话)号码,不严格模式*/
        TEL: "^1\\d{10}$", /*手机号码*/
        MOBILE: "^(0\\d{2,3}-?)\\d{4,8}(-\\d{1,4})?$|^(\\(0\\d{2,3}\\))\\d{4,8}(-\\d{1,4})?$|^\\d{4,8}$|^\\d{1,4}$", /*座机号码*/
        TELORMOBILE: "^1\\d{10}$|^(0\\d{2,3}-?)\\d{4,8}(-\\d{1,4})?$|^\\d{1,4}$", /*(手机或座机)号码,严格模式*/
        EMAIL: "^[a-z0-9._%-]+@([a-z0-9-]+\\.)+[a-z]{2,4}$", /*邮箱*/
        NUM: "^\\-?\\d+(\\.\\d+)?$",
        "prompt": {
            radio: "请选择一个选项",
            ...
            multiple: "多条数据使用逗号分隔",
            /*统一提示语*/
            phone: "请输入正确格式的电话号码,如(xxx)xxxxxxx,xxx-xxxxxxx,xxxxxxxxxxx,xxx-xxxxxxx-xxx",
            tel: "请输入正确的11位手机号码",
            mobile: "请输入正确格式的座机号码,如(xxx)xxxxxxx,xxx-xxxxxxx,xxx-xxxxxxx-xxxx",
            email: "请输入正确的邮箱格式,如xxx@xx.xx",
            telormobile: "请输入正确的座机或者手机号码",
            num: "该项只能填写数字"
        }
    };
    ...
    $.html5Validate = (function () {
        ...
        return {
            ...
            isRegex: function (ele, regex, params) {
                ...
                var parami = $(ele).attr("data-params");
                if(parami && parami == "ni") {
                	 var newRegExp = new RegExp(regex);
                } else{
                	 var newRegExp = new RegExp(regex, params || "");
                }
            ...
            isOverflow: function (ele) {
                if (!ele) return false;
                //  大小限制 长度限制 值
                var attrMin = $(ele).attr("min"), attrMax = $(ele).attr("max"), attrStep, attrDataMin, attrDataMax, value = ele.value;
                if (!attrMin && !attrMax) {
                    var label = $("label[for='" + ele.id + "']").text().replace(/\*|:|：/g, "");
                    attrDataMin = $(ele).attr("data-min"), attrDataMax = $(ele).attr("data-max");
                    if (attrDataMin && value.length < attrDataMin) {
                        $(ele).testRemind(label + "至少输入" + attrDataMin + "个字符");
                        ele.focus();
                    } else if (attrDataMax && value.length > attrDataMax) {
                        $(ele).testRemind(label + "最多输入" + attrDataMax + "个字符");
            ...
            isAllpass: function (elements, options) {
                ...
                var allpass = true, remind = function (control, type, tag) {
                    ...
                    var error_msg = $(control).attr("error-msg");
                    ...
                    if ($(control).isVisible()) {
                        if (type == "radio" || type == "checkbox") {
                            ...
                            var remiderStr = "";
                            if (text === "") {
                                remiderStr = OBJREG.prompt[tag];
                            } else {
                                remiderStr = "请选择" + text;
                            }
                            // 下拉值为空或文本框文本域等为空
                            $(control).testRemind((tag == "empty" && text) ? "您尚未输入" + text : remiderStr);
                            control.focus();
                        } else if (tag == "empty") {
                            // 下拉值为空或文本框文本域等为空
                            ...
                            // 文本框文本域格式不准确
                            var finalText = "";
                            if (OBJREG.prompt[type.toLowerCase()]) {
                                finalText = OBJREG.prompt[type.toLowerCase()];
                            } else if (OBJREG.prompt["pattern"]) {
                                finalText = "您输入的" + text + "格式不正确";
                            }
                            if (error_msg) {
                                finalText = error_msg;
                            }
    ...
    $.fn.extend({
        ...
        testRemind: function (content, options) {
            var defaults = {
                ...
                css: {
                    ...
                    borderColor: "rgb(247, 206, 57)",
                    color: "#333",
                    fontSize: "13.4px",
                    padding: "8px 15px",
                    zIndex: 5000,
                    boxShadow: 'rgba(107, 107, 107, 0.38) 0px 0px 22px 2px',
                    borderRadius: '5px'
                }
            };
            options = options || {};
            ...
            var objAlign = {"center": "50%", "left": "15%", "right": "85%"}, align = objAlign[params.align] || "50%";
        ...
        html5Validate: function (callback, options) {
            ...
            // 数值类型直接控制只能输入数字
            if (params.requiredNumVal) {
                var $form = $(this);
                $form.find("input[type=onlynum]").each(function () {
                    var $input = $(this);
                    //关联绑定事件,一般情况也不会用
                    var $span = $form.find('span[name=' + $input.attr('name') + ']');
            ...
            $(this).unbind("submit").bind("submit", function (event) {
                var elements = $(this).find(":input:visible");//选取页面显示的元素进行验证
